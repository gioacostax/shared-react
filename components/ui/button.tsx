...
> = ({ children, className, theme = 'primary', variant = 'box', ...rest }, ref) => (
  <button
    ...
    type="button"
    className={[
      `z-[1] box-border w-fit cursor-pointer whitespace-nowrap text-xs font-bold
      transition-all focus:outline focus:outline-2 focus:outline-offset-2 focus:outline-blue-500 disabled:cursor-not-allowed
      disabled:opacity-50`,
      variant === 'text' ? 'min-h-fit p-0' : 'min-h-10 rounded-md px-3 py-2',
      ...
      variant === 'outline' &&
        theme === 'secondary' &&
        `border-slate-700 text-slate-700 hover:border-slate-950 dark:border-slate-100
        dark:text-slate-100 dark:hover:border-slate-300`,
      variant === 'outline' &&
        theme === 'green' &&
